...
__inline__ static void generate_ulid(struct ulid *ulid) {
  ...
  // This line is storing the 64-bit random number generated by the pg_strong_random
  // function in the randomness variable in the last 8 bytes of the ulid->data array.
  // It is using pointer arithmetic to calculate the memory address of the 8-byte region
  // immediately following the timestamp bytes in the ulid->data array, and then casting
  // that memory address to a pointer to a 64-bit integer. Finally, it is dereferencing
  // that pointer and assigning the value of randomness to the memory location pointed
  // to by the pointer.
  *((int64_t *)(ulid->data + sizeof(timestamp))) = randomness;
}
