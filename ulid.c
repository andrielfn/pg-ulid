...
__inline__ static void generate_ulid(struct ulid *ulid) {
  ...
  // This code generates a 64-bit random number using the pg_strong_random
  ...
  // generator. The generated number is stored in the 'randomness' variable,
  // which is an int64_t (64-bit signed integer). The size of the
  // 'randomness' variable is passed as the second argument to the
  // pg_strong_random function, which is sizeof(randomness) in this case.
  ...
  // Set the last 8 bytes of the ULID to the randomness
  // This is done by copying the 8-byte randomness value to the last 8 bytes
  // of the ULID data array
